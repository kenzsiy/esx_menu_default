				...
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'{{#isSlider}} <span style="float:right;padding-right:10px;"><span class="slide"><</span>  <span class="amount">{{{sliderLabel}}}</span>  <span class="slide">></span></span>{{/isSlider}} {{{label}}}' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
			'</div>' +
		...
		for (let namespace in ESX_MENU.opened) {
			for (let name in ESX_MENU.opened[namespace]) {
				...
				for (let i=0; i<menuData.elements.length; i++) {
					...
					switch (element.type) {
						case 'default': {sliderOn = false;} break;
